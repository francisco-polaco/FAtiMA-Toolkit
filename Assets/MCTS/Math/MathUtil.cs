...
namespace MCTS.Math
{
    ...
    public static class MathUtil
    {
        ...
        public const float Deg2Radf = (float)(System.Math.PI / 180.0);
        public const float Rad2Degf = (float)(180.0 / System.Math.PI);
        ...
        public static bool EpsilonEqual(float a, float b, float epsilon = MathUtil.Epsilonf) {
            return (float)System.Math.Abs(a - b) <= epsilon;
        }
        ...
        {
            return Clamp(fValue, -System.Math.Abs(fMinMaxValue), System.Math.Abs(fMinMaxValue));
        }
        ...
        public static float SignedClamp(float f, float fMax) {
            return Clamp(System.Math.Abs(f), 0, fMax) * System.Math.Sign(f);
        }
        ...
        public static float SignedClamp(float f, float fMin, float fMax) {
            return Clamp(System.Math.Abs(f), fMin, fMax) * System.Math.Sign(f);
        }
        ...
        public static void MinMax(double a, double b, double c, out double min, out double max)
        {
            if ( a < b ) {
                if ( a < c ) {
                    min = a; max = System.Math.Max(b, c);
                } else {
                    min = c; max = b;
                }
            } else {
                if ( a > c ) {
                    max = a; min = System.Math.Min(b, c);
                } else {
                    min = b; max = c;
                }
            }
        ...
        //    }
        //    float fSign = System.Math.Sign(c[nPlaneNormalIdx]);
        ...
        //    }
        //    double fSign = System.Math.Sign(c[nPlaneNormalIdx]);
        ...
        //        return vFrom.Dot(vTo) < 0 ? 180.0f : 0;
        //    }
        //    float fSign = System.Math.Sign(Vector3f.Dot(c, planeN));
        //    float fAngle = fSign * Vector3f.AngleD(vFrom, vTo);
        //    return fAngle;
        //}
        ...
        //        return vFrom.Dot(vTo) < 0 ? 180.0 : 0;
        //    }
        //    double fSign = System.Math.Sign(Vector3d.Dot(c, planeN));
        //    double fAngle = fSign * Vector3d.AngleD(vFrom, vTo);
        //    return fAngle;
        //}
        ...
        public static float PlaneAngleSignedD(Vector2f vFrom, Vector2f vTo)
        {
            vFrom.Normalize();
            vTo.Normalize();
            float fSign = System.Math.Sign(vFrom.Cross(vTo));
        ...
        public static double PlaneAngleSignedD(Vector2d vFrom, Vector2d vTo)
        {
            vFrom.Normalize();
            vTo.Normalize();
            double fSign = System.Math.Sign(vFrom.Cross(vTo));
        ...
        //    double dot0 = System.Math.Abs(f.X.Dot(vDir));
        //    double dot1 = System.Math.Abs(f.Y.Dot(vDir));
        //    double dot2 = System.Math.Abs(f.Z.Dot(vDir));
        //    double m = System.Math.Max(dot0, System.Math.Max(dot1, dot2));
        ...
        {
            double denom = System.Math.Pow((fX - (xZero - 1)), speed);
        ...
        public static float WyvillFalloff(float fD, float fInnerRad, float fOuterRad)
        {
            if (fD > fOuterRad) {
                ...
                fD = System.Math.Max(0, System.Math.Min(1, fD));
  ...
  //      }
		...
		//		return 0;
		//	else
		//		return fDot / System.Math.Sqrt( d );
		//}
		...
		//	if ( d == 0 )
		//		return 0;
		//	return System.Math.Sqrt(d) / fDot;
		//}
		...
		//{
		//	return System.Math.Sign( ( (P1.x - P0.x) * (P2.y - P0.y) - (P2.x - P0.x) * (P1.y - P0.y) ) );
		//}
  ...
  //          return 2.0 * System.Math.Atan2(top, bottom);
  //      }
        ...
        public static bool SolveQuadratic(double a, double b, double c, out double minT, out double maxT)
        {
            ...
            double t = -0.5 * (b + System.Math.Sign(b)*System.Math.Sqrt(discrim));  
            minT = t / a;
            maxT = c / t;
            if ( minT > maxT ) {
